...
public class PostTagRefDAOImpl implements PostTagRefDAO{
	...
	private static final String FINDBY_PTR_POST_ID = "SELECT * FROM POST_TAG_REF p join TAG t on t.TAG_ID =p.PTR_TAG_ID WHERE PTR_POST_ID=?";
...
//throw new RuntimeException("A database error occured. " + se.getMessage());
	...
	public List<PostTagRefJoinVO> findBy_PTR_Post_Id(Integer ptr_post_id) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
    //	FINDBY_PTR_POST_ID ="SELECT * FROM POST_TAG_REF p join TAG t on t.TAG_ID =p.PTR_TAG_ID WHERE PTR_POST_ID=?";
		try {
			con = ds.getConnection();
